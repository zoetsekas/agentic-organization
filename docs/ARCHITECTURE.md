...
| Concurrency, driven | Two people on one design | **Exercised** (`scripts/concurrency_check.py`): seventeen checks over two browser contexts, all passing with nothing to fix. The non-holder's form goes fully read-only and the server refuses with a 409 naming the holder; an abandoned lock expires; independent edits merge and the same field conflicts with both values and a choice that the server honours. The one thing it proved about us rather than the product: a harness that reloads a page without re-asserting identity silently becomes one user and will report that locking is broken |

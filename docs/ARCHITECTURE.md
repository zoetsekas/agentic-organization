...
| The designer's data model | Nothing declared that nothing reads | **Audited.** Four dead declarations found after `CanvasNode.kind`'s closed set cost a save: `lock_break_requires` (a security setting that configured nothing), `Layout.viewport` (persisted, restored by nothing), `CanvasEdge`/`Layout.edges` (a second source of truth only the seeder wrote), `CanvasNode.color`. Two wired, two removed. The settings endpoint also validated nothing — `model_copy(update=...)` skips validation — so a typed model accepted anything and failed as a 500 from the store |
| Concurrency, driven | Two people on one design | **Exercised** (`scripts/concurrency_check.py`): 26 checks over two browser contexts. The non-holder's form goes fully read-only and the server refuses with a 409 naming the holder; an abandoned lock expires; independent edits merge and the same field conflicts with both values and a choice the server honours; breaking a lock takes it and is audited with who lost it. **The person who loses a lock is not told** — no heartbeat, nothing pushed — so they find out at save time, after the work (WS-032 M10) |

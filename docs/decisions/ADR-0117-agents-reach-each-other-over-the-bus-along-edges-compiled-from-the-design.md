...
# ADR-0117: Agents reach each other over the bus along edges compiled from the design, and the sender, the broker and the receiver each refuse the rest
...
3. **The broker enforces the same edges.** The local target renders
   ...
   reply-inbox prefix `_INBOX_<agent>.>`. Passwords are `$VAR` references
   resolved from the broker's environment and generated into the git-ignored
   `.env`; each worker is given its own. A one-shot `bus-init` service, as a
   bus operator no agent can act as, creates the tenant stream
   (`ORGAGENTS_<TENANT>` on `orgagents.<tenant>.agent.>`) and one durable pull
   consumer per agent; workers start after it has succeeded.
...
6. **Separation of duties holds across the chain.** Every hop carries the
   ...
   (`inputs.decision`) is refused, by sender and receiver, when anyone in the
   chain holds a decision the same separation names. And when the receiver
   runs the task, any tool that constitutes such a decision is replaced by a
   refusal for that run — so the buyer asking the COO to have payables pay is
   refused whether or not the request names `pay_invoice`.
...
- `src/orgagents/runtime/agent_bus.py` — `LinkPolicy` (the checks),
  ...
  `NatsTransport` (nats-py on its own event loop), `bus_init`.
...
## Disadvantages
- **The chain is a claim.** The immediate sender is vouched for by the broker;
  the principals before it are what that sender says. A compromised middle
  agent could drop the buyer from the chain. The receiver always adds the
  immediate sender, and the sender's own worker checked the chain it held,
  but end-to-end proof would need signed hops.
- **A delegation that does not name its decision** is only caught when the
  receiver reaches for a tool that constitutes one; a decision taken without a
  tool (a judgement in prose) is not a tool call and is not caught.
- The broker holds every agent's password (plain `$VAR`s, not bcrypt hashes or
  NKeys); a compromised broker container is every agent's bus identity.
  Moving to NKeys or hashed passwords is a follow-up.
...
real `nats-server` with the generated configuration (skipped, with the reason,
when neither a `nats-server` nor Docker is available).
`examples/ayc/end_to_end_messaging.py` runs it on the Docker stack.

...
### Dependencies, lint, types, coverage

- **Constraints.** `constraints.txt` pins every version CI and the designer
  image install. It is generated by [uv](https://docs.astral.sh/uv/)
  (`uv pip compile`, universal markers, Python 3.10 floor) from
  `pyproject.toml`: `make lock` (`pip install uv` first). Install with it the
  way CI does: `PIP_CONSTRAINT=constraints.txt pip install -e ".[dev,mcp]"`.
  Changing a range in `pyproject.toml` means re-running `make lock` in the
  same commit. Every 0.x/1.x framework has an upper bound at the line it was
  tested on; raising one is a tested change.
- **Lint.** `make lint` (ruff: pyflakes, pycodestyle, bugbear, import order;
  config in `pyproject.toml`). `ruff check --fix` sorts imports. Findings that
  predate ruff are listed per file under `extend-per-file-ignores`; remove an
  entry when you clean a file, never add one.
- **Types.** `make types` (mypy over `src/orgagents`). Strict-ish on `spec`,
  `metamodel`, `security` and `persistence` (every def annotated), lenient
  elsewhere. The per-module `disable_error_code` overrides at the end of
  `pyproject.toml` are the baseline; the same rule applies.
- **Coverage.** `make cov` runs the suite with pytest-cov and
  `scripts/coverage_floor.py`, which fails when a package drops below the
  floor measured for it. CI runs it on ubuntu/3.12 (with Docker, so the
  PostgreSQL suite counts).
- **Parallel.** `pytest -n auto --dist loadfile` (pytest-xdist) is what CI
  runs; each worker starts its own PostgreSQL container (or, against
  `ORGAGENTS_TEST_DATABASE_URL`, clones its own template database).

### Windows notes

- **tzdata.** Windows has no IANA time-zone database, so
  `zoneinfo.ZoneInfo("Europe/London")` needs the `tzdata` package; the
  project depends on it on Windows (`tzdata; sys_platform == 'win32'`).
- **Encoding.** `orgagents` and the test suite read and write their files as
  UTF-8 explicitly, so neither needs `PYTHONUTF8=1`; CI runs the suite on
  `windows-latest` without it. New code that opens a text file passes
  `encoding="utf-8"` (`Path.read_text`/`write_text` included).
- **Docker on Windows runners.** The PostgreSQL and NATS tests need a Linux
  container daemon; they skip, with the reason, where Docker is absent or runs
  Windows containers (as on GitHub's `windows-latest`). Docker Desktop's
  default Linux engine runs them.

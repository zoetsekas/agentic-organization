...
| M5 Audit log of designer actions | Phase 3 | Not started |
...
- Designer actions are not yet audited (M5), so "who changed this" relies on
  revision authorship alone.

## Exit criteria
- Identity comes from a real provider, with group-to-role mapping (M3).
- Every designer action is attributable in an audit log (M5).
